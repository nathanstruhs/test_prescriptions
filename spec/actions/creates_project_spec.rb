...
describe CreatesProject do
  it "creates a project give a name" do
  ...
  describe "task string parsing" do
    ...
    describe "handles a single string" do 
      let(:task_string) { "Start something" }
      specify { expect(tasks.size).to eq(1) }
      specify { expect(tasks.map(&:title)).to eq(["Start something"]) }
      specify { expect(tasks.map(&:size)).to eq([1]) }
    end

    describe "handles a single string with size" do
      let(:task_string) { "Start something:3" }
      specify { expect(tasks.size).to eq(1) }
      specify { expect(tasks.map(&:title)).to eq(["Start something"]) }
    ...
    describe "handles multiple tasks" do
      ...
      specify { expect(tasks.map(&:title)).to eq(["Start something", "End something"]) }
